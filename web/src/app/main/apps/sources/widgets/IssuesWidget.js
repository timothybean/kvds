...
function IssuesWidget() {
  const { data, title } = useSelector((selectWidgets => selectWidgets.IssuesWidget || {}));
  //const { data, title } = widgets
  ...
  return (
    <Paper className="flex flex-col flex-auto shadow rounded-2xl overflow-hidden">
      ...
      <Typography
        ...
        color="text.secondary"
      >
        <span className="truncate">{data?.extra && data.extra.name}</span>:
        <b className="px-8">{data?.extra && data.extra.count}</b>
      </Typography>
    </Paper>
  );
}
