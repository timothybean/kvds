...
type apiSnapshotServer interface {
	apiSnaphotsList() (*defs.APISnapshotList, error)
	apiSnapshotsGet(name string) (*defs.APISnapshotsGet, error)
}
...
func newAPI(
	...
	group.GET("/v3/config/source/list", a.onConfigPathsList)
	group.GET("/v3/config/sources/get/*name", a.onConfigPathsGet)
	group.POST("/v3/config/sources/add/*name", a.onConfigPathsAdd)
	group.PATCH("/v3/config/sources/patch/*name", a.onConfigPathsPatch)
	group.POST("/v3/config/sources/replace/*name", a.onConfigPathsReplace)
	group.DELETE("/v3/config/sources/delete/*name", a.onConfigPathsDelete)

	group.GET("/v3/sources/list", a.onPathsList)
	group.GET("/v3/sources/get/*name", a.onPathsGet)

	group.GET("/v3/config/groups/list", a.onConfigPathsList)
	group.GET("/v3/config/groups/get/*name", a.onConfigPathsGet)
	group.POST("/v3/config/groups/add/*name", a.onConfigPathsAdd)
	group.PATCH("/v3/config/groups/patch/*name", a.onConfigPathsPatch)
	group.POST("/v3/config/groups/replace/*name", a.onConfigPathsReplace)
	group.DELETE("/v3/config/groups/delete/*name", a.onConfigPathsDelete)
	...
	if !interfaceIsEmpty(a.snapServer) {
		group.GET("/v3/snapshots/list", a.onSnapshotsList)
		group.GET("/v3/snapshots/add", a.onWebRTCSessionsList)    //
		group.GET("/v3/snapshots/remove", a.onWebRTCSessionsList) //
		group.GET("/v3/snapshots/update", a.onWebRTCSessionsList) //
		group.GET("/v3/snapshots/get/:id", a.onSnapshotsGet)
	}
...
func (a *api) OnConfigGetGroups(ctx *gin.Context) {
	a.mutex.Lock()
	c := a.conf.Groups
	a.mutex.Unlock()
...
// Snapshots
func (a *api) onSnapshotsList(ctx *gin.Context) {
	data, err := a.snapServer.apiSnaphotsList()
	if err != nil {
		a.writeError(ctx, http.StatusBadRequest, err)
		return
	}
	...
	if data.Err != nil {
		ctx.JSON(http.StatusOK, data.Err.Error())
		return
	}
...
func (a *api) onSnapshotsGet(ctx *gin.Context) {
	name := ctx.Param("id")
	if name == "" {
	...
	data, err := a.snapServer.apiSnapshotsGet(name)
	if err != nil {
		a.writeError(ctx, http.StatusBadRequest, err)
		return
	}
	...
	err = jpeg.Encode(ctx.Writer, data.Image, &jpeg.Options{Quality: 60})
	if err != nil {
		a.Log(logger.Error, "Error generating JPEG image: %s", err.Error())
	}
}
