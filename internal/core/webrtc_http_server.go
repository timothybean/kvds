...
import (
	...
	"github.com/noranetworks/kvds/internal/protocols/webrtc"
...
func (s *webRTCHTTPServer) checkAuthOutsideSession(ctx *gin.Context, path string, publish bool) bool {
	...
	if res.err != nil {
		if terr, ok := res.err.(*errAuthentication); ok {
			if !hasCredentials {
				ctx.Header("WWW-Authenticate", `Basic realm="kvds"`)
...
func (s *webRTCHTTPServer) onWHIPPost(ctx *gin.Context, path string, publish bool) {
	...
	sourcePath, err := s.pathManager.getSourceForPath(path, "WEBRTC")
	if err != nil {
		webrtcWriteError(ctx, http.StatusNoContent, err)
		return
	}
