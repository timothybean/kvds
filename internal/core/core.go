...
import (
	...
	"github.com/noranetworks/kvds/internal/confwatcher"
	"github.com/noranetworks/kvds/internal/externalcmd"
	"github.com/noranetworks/kvds/internal/logger"
	"github.com/noranetworks/kvds/internal/record"
	"github.com/noranetworks/kvds/internal/rlimit"
)
...
type Core struct {
	...
	//transManager    *TranscoderManager
...
func New(args []string) (*Core, bool) {
	parser, err := kong.New(&cli,
		kong.Description("KVDS "+version),
		kong.UsageOnError(),
		kong.ValueFormatter(func(value *kong.Value) string {
			switch value.Name {
			case "confpath":
				return "path to a config file. The default is kvds.json"
...
func (p *Core) createResources(initial bool) error {
	...
	if initial {
		p.Log(logger.Info, "KVDS %s", version)
	...
	if p.snapshotManager == nil {
		p.snapshotManager = newSnapshotManager(
			p.ctx,
			p.conf.SnapShots,
			p.pathManager,
			p.metrics,
		...
		p.pathManager.setSnapshotManager(p.snapshotManager)
	}
...
func (p *Core) closeResources(newConf *conf.Conf, calledByAPI bool) {
	...
	if !closePathManager && !reflect.DeepEqual(newConf.Groups, p.conf.Groups) {
		p.pathManager.confGroupReload(newConf.Groups)
	}
	...
	// TODO: Update for closure on config change
	if p.snapshotManager != nil {
		p.snapshotManager.close()
		p.snapshotManager = nil
	}
