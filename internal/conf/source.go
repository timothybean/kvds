...
func (pconf *Source) setSourceDefaults() {
	// General
	...
	pconf.RPICameraTextOverlay = "%Y-%m-%d %H:%M:%S - KVDS" */
...
func newSource(defaults *Path, partial *OptionalPath) *Source {
	pconf := &Source{}
...
func (pconf *Source) check(conf *Conf, name string) error {
	pconf.Name = name
...
func (pconf *Source) Equal(other *Path) bool {
	return reflect.DeepEqual(pconf, other)
}

// HasStaticSource checks whether the path has a static source.
func (sconf Source) HasStaticSource() bool {
	return strings.HasPrefix(sconf.Source, "rtsp://") ||
		strings.HasPrefix(sconf.Source, "rtsps://") ||
		strings.HasPrefix(sconf.Source, "rtmp://") ||
		strings.HasPrefix(sconf.Source, "rtmps://") ||
		strings.HasPrefix(sconf.Source, "http://") ||
		strings.HasPrefix(sconf.Source, "https://") ||
		strings.HasPrefix(sconf.Source, "udp://") ||
		strings.HasPrefix(sconf.Source, "srt://") ||
		strings.HasPrefix(sconf.Source, "whep://") ||
		strings.HasPrefix(sconf.Source, "wheps://") ||
		sconf.Source == "rpiCamera"
}

// HasOnDemandStaticSource checks whether the path has a on demand static source.
func (sconf Source) HasOnDemandStaticSource() bool {
	return sconf.HasStaticSource() && sconf.SourceOnDemand
}

// HasOnDemandPublisher checks whether the path has a on-demand publisher.
func (sconf Source) HasOnDemandPublisher() bool {
	return sconf.RunOnDemand != ""
}
