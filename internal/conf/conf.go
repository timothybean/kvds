...
import (
	...
	"github.com/noranetworks/kvds/internal/conf/decrypt"
	"github.com/noranetworks/kvds/internal/conf/env"
	"github.com/noranetworks/kvds/internal/conf/yaml"
...
type Conf struct {
	...
	// Source defaults
	SourceDefaults Source `json:"pathDefaults"`
	...
	Transcoders map[string]*Transcoder `json:"transcoders"`
}
...
func (conf *Conf) setDefaults() {
	...
	conf.SourceDefaults.setSourceDefaults()
}
...
func (conf *Conf) loadFromFile(fpath string, defaultConfPaths []string) (string, error) {
	...
	if key, ok := os.LookupEnv("KVDS_CONFKEY"); ok {
		byts, err = decrypt.Decrypt(key, byts)
		if err != nil {
			return "", err
		}
